...
import cn.imhtb.zone.R;
import cn.imhtb.zone.callback.PromptCallback;
import cn.imhtb.zone.module.Module;
import cn.imhtb.zone.view.ARControllerManager;
import cn.imhtb.zone.view.PointsView;
import cn.imhtb.zone.view.ScanView;
        ...
        switch (state) {
            ...
            case MsgField.MSG_ON_QUERY_RESOURCE_ERROR_SERVER:
                UiThreadUtil.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(getContext(), (String) msg, Toast.LENGTH_SHORT)
                                .show();
                    }
                });
                break;
            ...
            case MsgField.MSG_PADDLE_GESTURE_ENABLE:
                UiThreadUtil.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(getContext(), "不支持手势能力！", Toast.LENGTH_SHORT)
                                .show();
                    }
                });
                break;
            case MsgField.MSG_PADDLE_IMG_SEG_ENABLE:
                UiThreadUtil.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(getContext(), "不支持背景分割能力", Toast.LENGTH_SHORT)
                                .show();
                    }
                });
                break;
