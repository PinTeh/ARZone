...
public class SpeechController {
     ...
     * 语音数据解析
     */
    public JSONObject parseVoiceData() {
        String absVocie = null;

        if (!TextUtils.isEmpty(ARConfig.getARPath())) {
            // 加载本地case
            absVocie = ARConfig.getARPath() + "/ar/res/voice.json";
        } else {
            // 下载case
            absVocie = ARFileUtils.getVoiceFilePath(ARConfig.getARKey());
        }
